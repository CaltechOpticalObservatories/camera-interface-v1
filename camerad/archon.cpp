...
namespace Archon {
  ...
  long Interface::configure_controller() {
    ...
    for (int entry=0; entry < this->config.n_entries; entry++) {
      ...
      // SCI_STOP_PARAM : parameter name to trigger science readout
      if (config.param[entry]=="SCI_STOP_PARAM") {
        this->sci_stop_param = config.arg[entry];
        message.str(""); message << "CONFIG:" << config.param[entry] << "=" << config.arg[entry];
        logwrite( function, message.str() );
        this->camera.async.enqueue( message.str() );
        applied++;
      ...
      // FCS_EXPTIME_MSEC_PARAM
      if (config.param[entry]=="FCS_EXPTIME_MSEC_PARAM") {
        this->fcs_exptime_msec_param = config.arg[entry];
        message.str(""); message << "CONFIG:" << config.param[entry] << "=" << config.arg[entry];
        logwrite( function, message.str() );
        this->camera.async.enqueue( message.str() );
        applied++;
      ...
      // SCI_EXPTIME_SEC_PARAM
      if (config.param[entry]=="SCI_EXPTIME_SEC_PARAM") {
        this->sci_exptime_sec_param = config.arg[entry];
        message.str(""); message << "CONFIG:" << config.param[entry] << "=" << config.arg[entry];
        logwrite( function, message.str() );
        this->camera.async.enqueue( message.str() );
        applied++;
   ...
   *
   */
  long Interface::power(const std::string args, std::string &retstring) {
    ...
    try {
      retstring = this->set_power(state);
    }
    catch (const std::exception &e) {
      logwrite(function, "ERROR: "+std::string(e.what()));
      return ERROR;
   ...
   *
   */
  std::string Interface::set_power(int state) {
    // must be connected
    if (!this->archon.isconnected()) {
      throw std::runtime_error("connection not open to controller");
    }
    ...
    switch( state ) {
      case 0:   // send POWEROFF command to Archon and wait 500ms to ensure off
                if ( this->archon_cmd(POWEROFF) == NO_ERROR ) {
                  std::this_thread::sleep_for(std::chrono::milliseconds(500));
                }
                break;
      case 1:   // send POWERON command to Archon and wait 2s to ensure stable
                if ( this->archon_cmd(POWERON) == NO_ERROR ) {
   ...
   *
   */
  std::string Interface::get_power() {
